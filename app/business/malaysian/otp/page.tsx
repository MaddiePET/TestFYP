...
export default function BusinessMalaysianOTP() {
  ...
  const handleBack = () => {
    ...
    if (step === "otp") {
      ...
      router.push(`/business/malaysian/face_verification?journeyId=${encodeURIComponent(journeyId)}`);
    }
  };
  ...
  const fetchAndRouteToInfo = async () => {
    try {
      ...
      router.push(`/business/malaysian/info?id_type=ic&id_num=${encodeURIComponent(icNo)}&journeyId=${encodeURIComponent(journeyId)}`);
    } catch (error) {
