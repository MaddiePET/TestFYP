...
export default function AdminLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const activeAccount = useMemo(() => {
    ...
    const found = initialAccounts.find((acc) => acc.name === currentAccountName);
    return found || null;
  }, [currentAccountName, loggedInUser]);
  ...
  return (
    <div className="min-h-screen xl:flex">
      {!mounted ? (
        ...
        >
          <div className={`py-8 flex lg:flex 
      ...
      <div className={`flex-1 transition-all duration-300 ease-in-out ${mainContentMargin}`}>
        ...
        >
          <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
            <div 
              className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4" 
                  ...
                  <div className="mt-4 pb-4 border-b border-gray-200 dark:border-gray-800">
                    ...
                    <ul className="flex flex-col gap-1">
                      {initialAccounts.map((account) => {
                        const isCurrent = currentAccountName === account.name;
                        const displayAvatar = isCurrent ? (activeAccount?.avatar || account.avatar) : account.avatar;

                        return (
                          <li key={account.id}>
                            <button
                              onClick={() => switchAccount(account.name)}
                              disabled={isProfilePage || isCurrent}
                            ...
                            >
                              <div className="overflow-hidden rounded-full h-9 w-9 shrink-0 border border-gray-200 dark:border-gray-700">
                                <img 
                                  className="w-full h-full object-cover" 
                                  src={displayAvatar || "/images/user/user-07.jpg"} 
                                  alt={account.name} 
                              ...
                              <div className="text-left">
                                <p className={`text-xs font-medium ${isCurrent ? 'text-[#3D405B] dark:text-white/80' : ''}`}>
                                  {account.name}
                                </p>

                                <p className={`text-[11px] ${isCurrent ? 'text-[#3D405B]/80 dark:text-white/80' : 'opacity-75'}`}>
                                  {account.email}
                                </p>
                              </div>
                              ...
                              {isCurrent && (
                                <svg 
                                  className="w-4 h-4 ml-auto shrink-0 fill-[#3D405B] dark:fill-white/80" 
                                  viewBox="0 0 24 24"
                                >
                                  <path 
                                    fillRule="evenodd" 
          ...
          {otpStep === 1 ? (
            ...
            <>
              <div className="text-center mb-10">
                ...
                {verificationMethod === "Email" ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    We've sent a 6-digit code to <span className="font-bold text-gray-900 dark:text-white">{targetAccountDetails?.email}</span>. Please provide the code to proceed with the switch.
                  </p>
                ) : (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    We've sent a 6-digit code to <span className="font-bold text-gray-900 dark:text-white">+60 {targetAccountDetails?.phone}</span>. Please provide the code to proceed with the switch.
                  </p>
                )}
              </div>
