...
export default function SavingsMalaysianAccountCreation() {
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      console.error("Malaysian savings account registration error:", error);
      setSubmitError(error.message || "Failed to complete savings account registration.");
    } finally {
  ...
  const handleBack = () => {
    if (step === "password") {
      ...
      router.push("/savings/malaysian/application");
    }
  };
