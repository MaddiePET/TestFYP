...
export default function MobilePassportCapture() {
  ...
  const handleCapture = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const okayidResponse = await fetch("/api/ekyc/okayid", {
        ...
        body: JSON.stringify({ journeyId, base64ImageString: base64String }),
      });
      const okayidResult = await okayidResponse.json();

      if (!okayidResponse.ok) throw new Error(okayidResult.error || "OCR Extraction Failed");
      ...
      if (!okaydocResponse.ok) throw new Error("Document Authentication Failed");
      ...
      console.error(error);
      alert(error.message || "Something went wrong. Please try taking the photo again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-[100dvh] px-4 pt-24 pb-6 bg-[#F9FAFB] dark:bg-gray-950 overflow-hidden">
      ...
      <header className="absolute top-6 left-4 right-4 flex justify-end items-center max-w-7xl mx-auto w-full z-20">
        ...
        {success ? (
          <div className="flex flex-col items-center w-full max-w-md animate-in fade-in zoom-in duration-500">
            ...
            <div className="mb-6 text-center">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Scan Successful!</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">Your passport has been securely verified.</p>
            </div>
          ...
          <div className="flex flex-col items-center w-full max-w-md animate-in fade-in zoom-in duration-500">
            ...
            >
              <span className="font-semibold text-sm">{isLoading ? "Verifying..." : "Open Camera"}</span>
              {isLoading ? (
                ...
                <svg className="w-6 h-6 text-[#3D405B] dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
