...
export default function PersonalNonMalaysianFaceVerification() {
  ...
  const handleBack = () => {
    if (scanIntervalRef.current) clearInterval(scanIntervalRef.current);
    stopCamera();
    router.push(`/personal/non-malaysian/passport?journeyId=${encodeURIComponent(journeyId)}`);
  };
      ...
      <div className="relative w-full max-w-md animate-in fade-in duration-500 z-10">
          ...
          {status === "scanning" && (
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <div className="z-20 p-6 text-center">
                <p className="mb-2 text-xl font-bold text-white drop-shadow-md">
                  {progress}%
                </p>
        ...
        <div className="space-y-3 relative z-10">
          ...
          {status === "scanning" && (
            <button 
              disabled 
              className="inline-flex items-center justify-center w-full px-4 py-3 text-sm font-bold transition rounded-lg shadow-theme-xs relative z-10 bg-gray-200 text-gray-400 cursor-not-allowed dark:bg-gray-800 dark:text-gray-600"
            >
              Capturing Face...
            </button>
          )}
