...
type Step = "option" | "input" | "otp";
type Method = "Email" | "Phone" | null;
type MessageType = "success" | "error" | "";

export default function PersonalNonMalaysianOTP() {
  ...
  const handleBack = () => {
    if (step === "otp") {
      ...
      router.push(`/personal/non-malaysian/face_verification?journeyId=${encodeURIComponent(journeyId)}`);
    }
  };
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    if (!journeyId) {
      setMessage("Journey ID missing. Please restart verification.");
  ...
  const fetchAndRouteToInfo = async () => {
    try {
      ...
      router.push(`/personal/non-malaysian/info?id_type=passport&id_num=${encodeURIComponent(passportNo)}&journeyId=${encodeURIComponent(journeyId)}`);
    } catch (error) {
  ...
  const handleVerifyOtp = async () => {
    ...
    if (method === "Email") {
      ...
      setTimeout(async () => {
        localStorage.setItem("nonMsianPhone", JSON.stringify({ ph_no: phoneNumber }));
        await fetchAndRouteToInfo();
      }, 800);
    }
  };
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-[100dvh] px-4 py-20 bg-[#F9FAFB] dark:bg-gray-950 overflow-hidden">
      ...
      <div className="relative w-full max-w-2xl z-10 animate-in fade-in duration-500">
        {step === "option" && (
          <>
