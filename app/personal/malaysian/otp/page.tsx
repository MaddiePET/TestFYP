...
export default function PersonalMalaysianOTP() {
  ...
  const handleBack = () => {
    ...
    if (step === "otp") {
      ...
      router.push(`/personal/malaysian/face_verification?journeyId=${encodeURIComponent(journeyId)}`);
    }
  };
  ...
  const fetchAndRouteToInfo = async () => {
    try {
      ...
      router.push(`/personal/malaysian/info?id_type=ic&id_num=${encodeURIComponent(icNo)}&journeyId=${encodeURIComponent(journeyId)}`);
    } catch (error) {
  ...
  const handleVerifyOtp = async () => {
    ...
    if (method === "Email") {
      try {
        ...
        localStorage.setItem("contactInfo", JSON.stringify({ email, emailVerified: true }));
      ...
      setTimeout(async () => {
        ...
        localStorage.setItem("contactInfo", JSON.stringify({ phone: activePhoneNumber, phoneVerified: true }));
        await fetchAndRouteToInfo();
      }, 800);
    }
  };
                ...
                <button 
                  ...
                  className={`inline-flex items-center justify-center w-full px-4 py-3 text-sm font-bold text-white transition rounded-lg bg-[#3D405B] hover:bg-[#2c2f42] disabled:bg-gray-200 disabled:text-gray-400 ${
                    method 
                      ...
                      : 'bg-gray-200 text-gray-400 cursor-not-allowed dark:bg-gray-00 dark:text-gray-400'
                  }`}
                >
                  Continue
                </button>
